...
SELECT COUNT(*) AS null_date_published
FROM movie
WHERE date_published IS NULL;

SELECT COUNT(*) AS null_duration
FROM movie
...
SELECT COUNT(*) AS null_worldwide_gross_income
FROM movie
WHERE worlwide_gross_income IS NULL;

SELECT COUNT(*) AS null_languages
FROM movie
WHERE languages IS NULL;

SELECT COUNT(*) AS null_production_company
FROM movie
...
SELECT 
    genre, COUNT(movie_id) AS Total_movie
FROM
    movie m
        INNER JOIN
...
(
	SELECT 
		movie_id, COUNT(genre) as no_of_genre
	FROM genre
	GROUP BY movie_id 
...
WITH genre_table AS 
(SELECT 
	genre, count(DISTINCT movie_id) AS movie_count,
...
ON r.movie_id = m.id
LIMIT 10;
-- There are 4 more movies having same rank as last one
...
(SELECT 
	production_company, COUNT(DISTINCT id) AS movie_count, 
...
SELECT 
    SUM(total_votes)/ COUNT(total_votes) AS German_votes_per_movie
FROM
    ratings r
        INNER JOIN
    movie m ON r.movie_id = m.id
WHERE
    LOWER(m.languages) LIKE '%german%';

SELECT 
    SUM(total_votes)/COUNT(total_votes) AS Itelian_votes_per_movie
FROM
    ratings r
        INNER JOIN
    movie m ON r.movie_id = m.id
WHERE
    LOWER(m.languages) LIKE '%italian%';
...
(
SELECT 
    name AS actor_name, SUM(total_votes)AS total_votes, COUNT(m.id) AS movie_count, 
...
INNER JOIN ratings r
    ON m.id = r.movie_id
WHERE lower(country) LIKE '%india%' and category = 'actor'
GROUP BY actor_name
...
FROM names n
    ...
    INNER JOIN ratings r
		ON m.id = r.movie_id
    WHERE languages LIKE '%hindi%' AND category = 'actress'
...
(
SELECT 
	genre, COUNT(m.id) AS Total_movie, RANK() OVER(ORDER BY count(m.id) DESC) AS Movie_rank
FROM 
	genre g 
LEFT JOIN 
movie m 
